defmodule ModelTest.Scenario.AnswerActor do
  ...
  def init(_) do
    ...
    state =
      ModelTest.Scenario.AnswerActorState.new()
  ...
  def handle_cast({:answer_question, message_id, content}, state) do
    ...
    actor_name = ModelTest.Scenario.AnswerActorState.get_state(state, "actor_name")
    env_pid = ModelTest.Scenario.AnswerActorState.get_state(state, "env_pid")
    GenServer.cast(env_pid, {:conversation_append, actor_name, answer})
    GenServer.cast(abs_actor_pid, {:response, message_id, {:question_reply, message_id, answer}})
    {:noreply, state}
  end
